	...
	if(dispatch.equalsIgnoreCase("Yes")){
		...
		<td  class="sorttable_nosort">
				<div id="report_heding" align="center"><font color="brown" size="3">TAT Report for <%
					...
					<a href="ExcelReceivedAndNoConfirmation.jsp?total=<%=total%>&from=<%=from %>&to=<%=to%>&dispatch=<%=dispatch%>"  title="Export to Excel"><img src="images/excel.jpg" width="15px" height="15px"></a> 
...
//					System.out.println("sql==>"+sql);
					...
					//	System.out.println("diffindays----->"+diffdays);
						...
						int QtCount=0;
						sql7="select count(*) as QtCount from db_CustomerComplaints.hwunittest where Unitid='"+unitid+"' and concat(TheDate,' ',TheTime)>=concat('"+Rdate+"',' ','"+Rtime+"') order by concat(TheDate,' ',TheTime) asc limit 1" ;
						System.out.println(sql7);
						 ...
						 if(rs7.next())
						 {
							...
							QtCount=rs7.getInt("QtCount");
							System.out.println(rs7.getString("QtCount"));
						 }
						...
						if(rsget1.next())
						{
							...
							receiveddate="-";
							if((receiveddate!=null && !(receiveddate.equals("-")))||((RecByTech.equalsIgnoreCase("yes"))||(RecByCust.equalsIgnoreCase("yes")))){}else{
								System.out.println("if unit is here it shouls print");
							...
							/*	 if(!(Rdate.equals("-")) && !(dispdate.equals("-")))
								...
								%>
								 ...
								 <tr>
									...
									<td><%=isWarranty%></td>
									<%
									int value5=(Long.valueOf(diffindays).intValue()*24)+(int)diffinhrs;
									%>
									<td><%=value5%>:<%=(int)min %></td>
									<td> <%=qtdate+" "+qttime%></td>
									<%
									if(qtFlag)
									{%>
									...
									%>
									<%
									int value4=(Long.valueOf(diffindaysqt).intValue()*24)+(int)diffinhrsqt;
									%>
									<td><%=value4%>:<%=(int)minqt %></td>
									...
									try{
										...
										int value3=(Long.valueOf(diffindayseditdt).intValue()*24)+(int)diffinhrsedit;
									%>
									
									<td><%=value3%>:<%=(int)mindt %></td>
									...
									<td><%=testedBy %></td>
									<%
									int value2=(Long.valueOf(diffindaystestdt).intValue()*24)+(int)diffinhrstest;
									%>
									<td><%=value2%>:<%=(int)mintest %></td>
									<%
									int value1=(Long.valueOf(diffindaysTotTat).intValue()*24)+(int)diffinhrsTotTat;
									%>
									<td><%=value1%>:<%=(int)mintot %></td>
									<td><%=dispdate+" "+disptime%></td>
									<td><%=dispdoneby %></td>
									<td><%=receiveddate+" "+receivetime %></td>
									<td><%=recname %></td>
									<%
									int value=(Long.valueOf(diffindaysdisp).intValue()*24)+(int)diffinhrsdisp;
									%>
									<td><%=value%>:<%=(int)mindisp %></td>
