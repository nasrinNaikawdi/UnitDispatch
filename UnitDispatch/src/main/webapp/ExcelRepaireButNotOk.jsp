	...
	if(dispatch.equalsIgnoreCase("Yes")){
		...
		<td  class="sorttable_nosort">
				<div id="report_heding" align="center"><font color="brown" size="3">TAT Report for <%
					...
					<a href="ExcelRepaireButNotOk.jsp?total=<%=total%>&from=<%=from %>&to=<%=to%>&dispatch=<%=dispatch%>"  title="Export to Excel"><img src="images/excel.jpg" width="15px" height="15px"></a> 
						...
						/*	 if(!(Rdate.equals("-")) && !(dispdate.equals("-")))
							...
							%>
							 ...
							 <tr>
								...
								<td><a href="#" onClick="window.open ('qt_details_window.jsp?unitid=<%=unitid %>&recDate=<%=Rdate%>&recTime=<%=Rtime %>&dispDate=<%=dispdate%>&dispTime=<%=disptime %>&qtcount=No','width=600, height=350, location=0, menubar=0, scrollbars=1, status=0, toolbar=0, resizable=0');"><%=QtCount %></font></a></td>
								<%
								}%>
								<td><%=testDateTime %></td>
								<td><%=testedBy %></td>
