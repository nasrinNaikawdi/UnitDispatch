	...
	*/
	...
	if(dispatch.equalsIgnoreCase("Yes")){
		...
		<div id="report_heding" align="center"><font color="brown" size="3">TAT Report for <%
			...
			<a href="ExcelForTatForReceivedButNotQT.jsp?total=<%=total%>&uidList=<%=unitID_list%>&from=<%=from %>&to=<%=to%>"  title="Export to Excel"><img src="images/excel.jpg" width="15px" height="15px"></a> 
...
//					System.out.println("sql==>"+sql);
					...
					//	System.out.println("diffindays----->"+diffdays);
						...
						if(rsget.next())
						{
						 ...
						 if(!(dispdate.equals("-")))
						 {
						 que1="Select * from t_unitmasterhistory where (RecDateTime >='"+dispdate+" 00:00:00' or RecByCustDate>='"+dispdate+" 00:00:00') and UnitId='"+unitid+"' and Status='Dispatched' and (RecByTech='Yes' OR RecByCust='Yes') and RecConfirmByRep!='-' order by RecDateTime Limit 1";
					...
					/*	 if(!(Rdate.equals("-")) && !(dispdate.equals("-")))
						...
						%>
						 ...
						 <tr>
							...
							<td><a href="#" onClick="window.open ('qt_details_window.jsp?unitid=<%=unitid %>&recDate=<%=Rdate%>&recTime=<%=Rtime %>&dispDate=<%=dispdate%>&dispTime=<%=disptime %>&qtcount=yes','width=600, height=350, location=0, menubar=0, scrollbars=1, status=0, toolbar=0, resizable=0');"><%=QtCount %></a></td>
							<%
							}%>
							<td><%=testDateTime %></td>
							<td><%=testedBy %></td>
	...
	%>
				...
				<div id="report_heding" align="center"><font color="brown" size="3">TAT Report for <%
					...
					<a href="TATReport_export.jsp?total=<%=total%>&uidList=<%=unitID_list%>&data=<%=from %>&data1=<%=to%>"  title="Export to Excel"><img src="images/excel.jpg" width="15px" height="15px"></a> 
					<%=new SimpleDateFormat("dd-MMM-yyyy HH:mm:ss").format(new java.util.Date())%>
					</div>
				</td>
				</tr>
