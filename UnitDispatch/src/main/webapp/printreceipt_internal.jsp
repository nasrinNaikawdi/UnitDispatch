...
<table border="0" width="100%">
   <tr> 
	...
	<td align="right"> <a href="exportreceipt_internal.jsp?calender=<%=sevdaybfre%>&calender1=<%=tdydte%>"> <img src="images/excel.jpg" width="15px" height="15px" border="0"/></a></u> &nbsp;&nbsp;&nbsp;
...
<%
   ...
   { 
      sql1="select distinct(OrderNo),OrderNo, DispDate from t_unitmasterhistory  where DispId <> '0' and Status='Sent' and testedunit='Yes' and DispDate between '"+sevdaybfre+"' and '"+tdydte+"' group by OrderNo order by DispDate desc";
      System.out.println(sql1);
   }
   else
   {
      sql1="select distinct(OrderNo),OrderNo DispDate from t_unitmasterhistory  where User='"+user+"' and Status='Sent' and testedunit='Yes' and DispId <> '0' and DispDate between '"+sevdaybfre+"' and '"+tdydte+"' group by OrderNo order by DispDate desc";
      System.out.println(sql1);
   }
int orderno=0;
...
<%    } 
   ...
   { 
      String acthalf="";
      acthalf=totunitid.substring(0,5);
%>
 <input type="hidden" id="fullaction<%=c %>" name="fullaction<%=c %>" value="<%=totunitid %>" > </input>  
...
<%    } 
   ...
   { 
      String acthalf="";
      acthalf=totdispid.substring(0,6);
%>
 <input type="hidden" id="fullaction<%=c %>" name="fullaction<%=c %>" value="<%=totdispid %>" > </input>  
<td> <%=acthalf %>.....<div id="sh<%=c %>" name="sh<%=c %>"><a href="#" ONMOUSEOVER="show(fullaction<%=c %>)"; ONMOUSEOUT="kill()">Show </a> </div> </td> 
...
<%	} %>
   ...
   <td> <a href="" onClick="newwindow('printinternalrecpt.jsp?orderno=<%=ordrno %>&orderqty=<%=ordqty %>&orderdt=<%=ordrdte %>&dispdtqty=<%=dispqty %>&dispdt=<%=dispdte %>&unitdisp=<%=totunitid %>&dispadd=<%=add%>&cust=<%=cust %>&modeofdisp=<%=mode %>&dispname=<%=modename %>&chalanno=<%=chalanno %>&dispid=<%=totdispid %>&orderby=<%=ordby %>&dispby=<%=dispby %>');return false" target="_parent"> Print</a> </td> 
